...
- 鼠标悬停通知按钮会打开简易通知栏，点击按钮可进入完整通知界面。
- 通知栏和通知界面都支持“全部已读”。未读通知项前显示红点，鼠标悬停显示“点击查看详情”。
- 点击通知会打开对应任务详情。
- 只提醒未完成任务：优先使用最后一条延期日期，没有延期时使用计划完成日期。
- 距期望完成时间小于 `7` 天时生成提醒，每天重建当天通知；到期当天和逾期通知使用专用文案。
- 通知按剩余天数升序排列。
- 通知数据独立保存在程序目录下的 `notifications.db`，后台异步刷新，不修改核心 `data.db`。
- 通知历史最多保留最近 `30` 条。
...
| `notification.mark_read` | 标记单条通知为已读 |
| `notification.mark_all_read` | 标记当天通知全部已读 |
...
│       ├── notifications.rs        # 独立通知数据库和异步刷新逻辑
