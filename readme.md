...
- 对未完成任务，后端和历史空值会统一显示为“进行中”；实际完成时间只有填写可解析的确切日期时才会认定为已完成，其他文本仍按未完成处理。
...
- 会议纪要号：模糊匹配。
- 任务序号：精确匹配。
- 责任部门：模糊匹配，多个值使用英文逗号分隔，值之间为“或”关系；筛选输入框仅显示完整的“责任部门”提示。
- 责任人：模糊匹配，多个值使用英文逗号分隔，值之间为“或”关系；筛选输入框仅显示完整的“责任人”提示。
- 计划完成时间：开始日期和结束日期。
- 实际完成时间：开始日期和结束日期。
- 延期时间：开始日期和结束日期，只要任一延期记录落入范围即可。
- 延期次数：筛选延期记录数量大于或等于指定值的任务。
- 附件状态：全部、有附件或无附件；主表附件列有附件时显示回形针图标，无附件时保持空白。
...
可选择的年份范围为 `2000` 至 `2100`。在按月显示的日期选择器中点击年份，会关闭当前按月视图并切换到按年份滚动的选择窗口；确认年份后自动返回按月日历，但不会直接修改最终日期。用户需要在按月日历中继续选择月份和日期，最终日期始终以按月日历中的选择为准。

### 通知与到期提醒

- 主界面通知按钮位于刷新按钮左侧；存在未读通知时按钮右上角显示红点。
- 鼠标悬停通知按钮会打开独立设计的紧凑预览组件，外层宽度为 `390px`、最高约 `498px`，标题栏高度保持原设计；预览不再固定限制为前 `2` 条，而是根据当前通知数量和正文高度动态调整面板高度，尽量填满可用空间，超出后可在面板内滚动查看全部通知。点击按钮可进入单独设计的完整通知界面。
...
- 紧凑通知栏和完整通知界面的右上角均使用相同的“全部已读”图标；完整界面同时显示“全部已读”文字，可一次标记当天全部通知。
...
- 主界面“历史快照”按钮使用点击菜单，可选择“保存历史快照”或“查看历史快照”。
- 保存时会弹出确认框，可填写可选备注；备注默认留空。保存成功后的底部提示会显示当前已使用的快照份数。
- 查看时会弹出快照选择框，显示保存时间和备注；点击任一快照后进入仿照主界面表格设计的只读子界面。
- 快照子界面支持表头排序，但不提供添加、编辑、删除、延期或附件修改操作。
...
- 所有业务输入框统一采用添加/编辑窗口的填充色、圆角边框、聚焦边框和提示文字风格；顶部搜索栏和筛选栏仅保留紧凑高度与搜索/筛选自身需要的图标，日期输入框继续保留右侧日历按钮。
- 历史快照只读子界面同样提供搜索栏和筛选按钮，搜索与筛选只作用于当前快照，不会修改主数据库或快照内容。
...
- 主界面“查看/删除延期”右侧提供“自动调整列宽”按钮，会按表头和当前数据显示内容重新计算列宽；各栏最小宽度按完整表头预留，保证表头不会被隐藏。
- 主表右侧垂直滚动条支持鼠标拖动，操作方式与浏览器滚动条一致；无滚动操作时自动隐藏，滚动时显示。
- 附件列锁定为仅能显示完整“附件”表头和回形针图标的最窄宽度，不提供拖动调整；有附件时图标水平居中，无附件时保持空白。
- 列宽有最小值限制，避免栏目被完全压缩。
...
- 自动保存并恢复窗口大小、位置、最大化状态和各列宽度；窗口最小宽度为 `1200px`，保证表头的最小显示空间。
...
| `snapshot.create` | 保存历史快照，可附带可选备注，并返回当前使用份数 |
| `snapshot.list` | 查询历史快照的保存时间和备注 |
| `snapshot.get` | 查询一份历史快照中的只读任务数据 |
...
│   │       ├── delay_screen.dart
│   │       ├── filter_screen.dart
│   │       ├── snapshot_screen.dart # 历史快照只读表格界面
